...
import { ITigerdUpdateService } from '../../workbench/contrib/tigerd/common/tigerdUpdateService.js';
import { MetricsMainService } from '../../workbench/contrib/tigerd/electron-main/metricsMainService.js';
import { TigerdMainUpdateService } from '../../workbench/contrib/tigerd/electron-main/tigerdUpdateMainService.js';
import { LLMMessageChannel } from '../../workbench/contrib/tigerd/electron-main/sendLLMMessageChannel.js';
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		services.set(ITigerdUpdateService, new SyncDescriptor(TigerdMainUpdateService, undefined, false));
		services.set(ITigerdSCMService, new SyncDescriptor(TigerdSCMService, undefined, false));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		mainProcessElectronServer.registerChannel('tigerd-channel-metrics', metricsChannel);

		const tigerdUpdatesChannel = ProxyChannel.fromService(accessor.get(ITigerdUpdateService), disposables);
		mainProcessElectronServer.registerChannel('tigerd-channel-update', tigerdUpdatesChannel);

		const sendLLMMessageChannel = new LLMMessageChannel(accessor.get(IMetricsService));
		mainProcessElectronServer.registerChannel('tigerd-channel-llmMessage', sendLLMMessageChannel);

		// Void added this
		const tigerdSCMChannel = ProxyChannel.fromService(accessor.get(ITigerdSCMService), disposables);
		mainProcessElectronServer.registerChannel('tigerd-channel-scm', tigerdSCMChannel);
