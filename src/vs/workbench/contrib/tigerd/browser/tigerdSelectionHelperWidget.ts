...
import { mountTigerdSelectionHelper } from './react/out/tigerd-editor-widgets-tsx/index.js';
...
export type TigerdSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.tigerdSelectionHelper';
	// react
	...
	constructor(
		...
		@ITigerdSettingsService private readonly _tigerdSettingsService: ITigerdSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountTigerdSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._tigerdSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies TigerdSelectionHelperProps)
