...
import { ITigerdCommandBarService } from './tigerdCommandBarService.js';
...
import { FeatureName } from '../common/tigerdSettingsTypes.js';
import { ITigerdModelService } from '../common/tigerdModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, TigerdFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js';
// import { isMacintosh } from '../../../../base/common/platform.js';
// import { VOID_OPEN_SETTINGS_ACTION_ID } from './tigerdSettingsPane.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@ITigerdSettingsService private readonly _settingsService: ITigerdSettingsService,
		// @IFileService private readonly _fileService: IFileService,
		@ITigerdModelService private readonly _tigerdModelService: ITigerdModelService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._tigerdModelService.initializeModel(model.uri)
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._tigerdModelService.getModel(uri)
	...
	private _getCurrentTigerdFileSnapshot = (uri: URI): TigerdFileSnapshot => {
		const { model } = this._tigerdModelService.getModel(uri)
	...
	private _restoreTigerdFileSnapshot = async (uri: URI, snapshot: TigerdFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: TigerdFileSnapshot = this._getCurrentTigerdFileSnapshot(uri)
		let afterSnapshot: TigerdFileSnapshot | null = null
		...
		const elt: IUndoRedoElement = {
			...
			undo: async () => { opts?.onWillUndo?.(); await this._restoreTigerdFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreTigerdFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentTigerdFileSnapshot(uri)
			await this._tigerdModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getTigerdFileSnapshot(uri: URI) {
		return this._getCurrentTigerdFileSnapshot(uri)
	}


	public restoreTigerdFileSnapshot(uri: URI, snapshot: TigerdFileSnapshot): void {
		this._restoreTigerdFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Tigerd: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._tigerdModelService.initializeModel(uri)
		await this._tigerdModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Tigerd: diff.type not recognized on: ${from}`)
		}
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Tigerd: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Tigerd error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@ITigerdCommandBarService private readonly _tigerdCommandBarService: ITigerdCommandBarService,
		...
		const getAcceptRejectText = () => {
			...
			const commandBarStateAtUri = this._tigerdCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._tigerdCommandBarService.onDidChangeState(e => {
