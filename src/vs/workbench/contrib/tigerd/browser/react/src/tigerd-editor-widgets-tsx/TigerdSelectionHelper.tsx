...
import '../styles.css'
import { TIGERD_CTRL_K_ACTION_ID, TIGERD_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { TigerdSelectionHelperProps } from '../../../../../../contrib/tigerd/browser/tigerdSelectionHelperWidget.js';
import { VOID_OPEN_SETTINGS_ACTION_ID } from '../../../tigerdSettingsPane.js';


export const TigerdSelectionHelperMain = (props: TigerdSelectionHelperProps) => {
	...
	>
		<TigerdSelectionHelper {...props} />
	</div>
}



const TigerdSelectionHelper = ({ rerenderKey }: TigerdSelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(TIGERD_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(TIGERD_CTRL_K_ACTION_ID)
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === TIGERD_CTRL_L_ACTION_ID || e.commandId === TIGERD_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(TIGERD_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(TIGERD_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
