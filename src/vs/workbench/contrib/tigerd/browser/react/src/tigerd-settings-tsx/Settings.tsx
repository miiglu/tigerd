...
const ConfirmButton = ({ children, onConfirm, className }: { children: React.ReactNode, onConfirm: () => void, className?: string }) => {
	...
	return (
		<div ref={ref} className={`inline-block`}>
			<TigerdButtonBgDarken className={className} onClick={() => {
				if (!confirm) {
					setConfirm(true);
				} else {
					onConfirm();
...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('ITigerdSettingsService')
	...
	const sourcecodeOverridesLink = `https://github.com/miiglu/void/blob/2e5ecb291d33afbe4565921664fb7e183189c1c5/src/vs/workbench/contrib/tigerd/common/modelCapabilities.ts#L146-L172`
		...
		>
			...
			>
				...
				<div className="text-sm text-void-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with Tigerd, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by Tigerd.`
							: `Tigerd recognizes ${modelName} ("${recognizedModelName}").`}
				</div>


				{/* override toggle */}
				<div className="flex items-center gap-2 mb-4">
					<TigerdSwitch size='xs' value={overrideEnabled} onChange={setOverrideEnabled} />
				...
				<div className="flex justify-end gap-2 mt-4">
					<TigerdButtonBgDarken onClick={onClose} className="px-3 py-1">
						Cancel
					</TigerdButtonBgDarken>
					<TigerdButtonBgDarken
						onClick={onSave}
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	const modelDump: (TigerdStatefulModelInfo & { providerName: ProviderName, providerEnabled: boolean })[] = []
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const handleChangeValue = useCallback((newVal: string) => {
		tigerdSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [tigerdSettingsService, providerName, settingName]);
...
// 	const tigerdSettingsState = useSettingsState()

// 	const needsModel = isProviderNameDisabled(providerName, tigerdSettingsState) === 'addModel'

// 	// const accessor = useAccessor()
// 	// const tigerdSettingsService = accessor.get('ITigerdSettingsService')

// 	// const { enabled } = tigerdSettingsState.settingsOfProvider[providerName]
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = tigerdSettingsService.state.settingsOfProvider[providerName].enabled
// 						tigerdSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [tigerdSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const TigerdProviderSettings = ({ providerNames }: { providerNames: ProviderName[] }) => {
	return <>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	const enabled = tigerdSettingsState.globalSettings[settingName]

	return <ButtonLeftTextRightOption
		leftButton={<TigerdSwitch
			size='xxs'
			value={enabled}
			onChange={(newVal) => {
				tigerdSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	const tigerdSettingsService = accessor.get('ITigerdSettingsService')
	const tigerdSettingsState = useSettingsState()
	return <TigerdInputBox2
		className='min-h-[81px] p-3 rounded-sm'
		initValue={tigerdSettingsState.globalSettings.aiInstructions}
		...
		onChangeText={(newText) => {
			tigerdSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		tigerdSettingsService.setGlobalSetting('enableFastApply', newVal)
	}, [tigerdSettingsService])

	return <TigerdCustomDropdownBox
		className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1'
		options={options}
		selectedOption={tigerdSettingsService.state.globalSettings.enableFastApply}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-void-fg-3 text-sm list-decimal select-text'>
		...
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`Tigerd automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	return <div
		...
		onClick={() => { tigerdSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		tigerdSettingsService.setGlobalSetting('autoApprove', {
			...tigerdSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [tigerdSettingsService, metricsService])

	return <>
		<TigerdSwitch
			size={size}
			value={tigerdSettingsState.globalSettings.autoApprove[approvalType] ?? false}
...
export const OneClickSwitchButton = ({ fromEditor = 'VS Code', className = '' }: { fromEditor?: TransferEditorType, className?: string }) => {
	...
	return <>
		<TigerdButtonBgDarken className={`max-w-48 p-4 ${className}`} disabled={transferState.type !== 'done'} onClick={onClick}>
...
const MCPServerComponent = ({ name, server }: { name: string, server: MCPServer }) => {
	...
	const tigerdSettings = useSettingsState()
	const isOn = tigerdSettings.mcpUserStateOfName[name]?.isOn
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			dataStr = JSON.stringify(tigerdSettingsService.state, null, 2)
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					tigerdSettingsService.dangerousSetState(json as any)
				}
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`Tigerd's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`Tigerd can access any model that you host locally. We automatically detect your local models by default.`}</h3>

									<div className='opacity-80 mb-4'>
										<OllamaSetupInstructions sayWeAutoDetect={true} />
									</div>

									<TigerdProviderSettings providerNames={localProviderNames} />
								</ErrorBoundary>
							</div>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`Tigerd can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>

									<TigerdProviderSettings providerNames={nonlocalProviderNames} />
								</ErrorBoundary>
							</div>
								...
								<ErrorBoundary>
									...
									<div className='flex flex-col gap-y-8 my-4'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<TigerdSwitch
																...
																onChange={(newVal) => tigerdSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<TigerdSwitch
															...
															onChange={(newVal) => tigerdSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<TigerdSwitch
															...
															onChange={(newVal) => tigerdSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<TigerdSwitch
															...
															onChange={(newVal) => tigerdSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-void-fg-3 mt-1'>{`Settings that control the visibility of Tigerd suggestions in the code editor.`}</div>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<TigerdSwitch
															...
															onChange={(newVal) => tigerdSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<TigerdSwitch
															...
															onChange={(newVal) => tigerdSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-void-fg-3 mb-4'>{`Transfer your editor settings into Tigerd.`}</h4>
								...
								<div>
									...
									<h4 className='text-void-fg-3 mb-4'>{`Transfer Tigerd's settings and chats in and out of Tigerd.`}</h4>
									<div className='flex flex-col gap-8'>
										{/* Settings Subcategory */}
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											<input key={2 * s} ref={fileInputSettingsRef} type='file' accept='.json' className='hidden' onChange={handleUpload('Settings')} />
											<TigerdButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputSettingsRef.current?.click() }}>
												Import Settings
											</TigerdButtonBgDarken>
											<TigerdButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Settings')}>
												Export Settings
											</TigerdButtonBgDarken>
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { tigerdSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<TigerdButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputChatsRef.current?.click() }}>
												Import Chats
											</TigerdButtonBgDarken>
											<TigerdButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Chats')}>
												Export Chats
											</TigerdButtonBgDarken>
								...
								<div>
									...
									<ErrorBoundary>
										<div className='flex flex-col gap-2 justify-center max-w-48 w-full'>
											<TigerdButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openSettings') }}>
												General Settings
											</TigerdButtonBgDarken>
											<TigerdButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openGlobalKeybindings') }}>
												Keyboard Settings
											</TigerdButtonBgDarken>
											<TigerdButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.selectTheme') }}>
												Theme Settings
											</TigerdButtonBgDarken>
											<TigerdButtonBgDarken className='px-4 py-1' onClick={() => { nativeHostService.showItemInFolder(environmentService.logsHome.fsPath) }}>
												Open Logs
											</TigerdButtonBgDarken>
										</div>
									</ErrorBoundary>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-void-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep Tigerd running smoothly. You may opt out below. Regardless of this setting, Tigerd never sees your code, messages, or API keys.</h4>
								...
								`} chatMessageLocation={undefined} />
									...
									<div className='my-4'>
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												<TigerdSwitch
													...
													onChange={(newValue) => {
														tigerdSettingsService.setGlobalSetting('disableSystemMessage', newValue);
													}}
												/>
										...
										<div className='text-void-fg-3 text-xs mt-1'>
											{`When disabled, Tigerd will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
...
Use Model Context Protocol to provide Agent mode with more tools.
							`} chatMessageLocation={undefined} />
									...
									<div className='my-2'>
										<TigerdButtonBgDarken className='px-4 py-1 w-full max-w-48' onClick={async () => { await mcpService.revealMCPConfigFile() }}>
											Add MCP Server
										</TigerdButtonBgDarken>
									</div>
