...
import { BlockCode, TextAreaFns, TigerdCustomDropdownBox, TigerdInputBox2, TigerdSlider, TigerdSwitch, TigerdDiffEditor } from '../util/inputs.js';
import { ModelDropdown, } from '../tigerd-settings-tsx/ModelDropdown.js';
import { PastThreadsList } from './SidebarThreadSelector.js';
import { TIGERD_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { VOID_OPEN_SETTINGS_ACTION_ID } from '../../../tigerdSettingsPane.js';
import { ChatMode, displayInfoOfProviderName, FeatureName, isFeatureNameDisabled } from '../../../../../../../workbench/contrib/tigerd/common/tigerdSettingsTypes.js';
import { ICommandService } from '../../../../../../../platform/commands/common/commands.js';
import { WarningBox } from '../tigerd-settings-tsx/WarningBox.js';
...
const ReasoningOptionSlider = ({ featureName }: { featureName: FeatureName }) => {
	...
	const modelSelection = tigerdSettingsState.modelSelectionOfFeature[featureName]
	const overridesOfModel = tigerdSettingsState.overridesOfModel
	...
	const modelSelectionOptions = tigerdSettingsState.optionsOfModelSelection[featureName][providerName]?.[modelName]
		...
		return <div className='flex items-center gap-x-2'>
			...
			<TigerdSwitch
				...
				onChange={(newVal) => {
					...
					tigerdSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff })
				}}
			/>
		</div>
	}
	...
	if (reasoningBudgetSlider?.type === 'budget_slider') { // if it's a slider
		...
		const value = isReasoningEnabled ? tigerdSettingsState.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]?.reasoningBudget ?? defaultVal
			: valueIfOff
		...
		return <div className='flex items-center gap-x-2'>
			...
			<TigerdSlider
				...
				onChange={(newVal) => {
					...
					tigerdSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningBudget: newVal })
				}}
			/>
	...
	if (reasoningBudgetSlider?.type === 'effort_slider') {
		...
		const currentEffort = tigerdSettingsState.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]?.reasoningEffort ?? defaultVal
		const valueIfOff = -1
		...
		return <div className='flex items-center gap-x-2'>
			...
			<TigerdSlider
				...
				onChange={(newVal) => {
					const isOff = canTurnOffReasoning && newVal === valueIfOff
					tigerdSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningEffort: values[newVal] ?? undefined })
				}}
			/>
...
const ChatModeDropdown = ({ className }: { className: string }) => {
	...
	const onChangeOption = useCallback((newVal: ChatMode) => {
		tigerdSettingsService.setGlobalSetting('chatMode', newVal)
...
export const TigerdChatArea: React.FC<TigerdChatAreaProps> = ({
	children,
	onSubmit,
	onAbort,
	onClose,
...
export const SelectedFiles = (
	...
	const accessor = useAccessor()
	const commandService = accessor.get('ICommandService')
	const modelReferenceService = accessor.get('ITigerdModelService')
...
const UserMessageComponent = ({ chatMessage, messageIdx, isCheckpointGhost, currCheckpointIdx, _scrollToBottom }: { chatMessage: ChatMessage & { role: 'user' }, messageIdx: number, currCheckpointIdx: number | undefined, isCheckpointGhost: boolean, _scrollToBottom: (() => void) | null }) => {
	...
	if (mode === 'display') {
		...
		chatbubbleContents = <TigerdChatArea
			featureName='Chat'
			onSubmit={onSubmit}
			onAbort={onAbort}
			isStreaming={false}
...
const EditToolChildren = ({ uri, code, type }: { uri: URI | undefined, code: string, type: 'diff' | 'rewrite' }) => {
	...
	const content = type === 'diff' ?
		<TigerdDiffEditor uri={uri} searchReplaceBlocks={code} />
...
export const SidebarChat = () => {
	...
	const keybindingString = accessor.get('IKeybindingService').lookupKeybinding(TIGERD_CTRL_L_ACTION_ID)?.getLabel()
	...
	const inputChatArea = <TigerdChatArea
		featureName='Chat'
		onSubmit={() => onSubmit()}
		onAbort={onAbort}
