...
import { displayInfoOfProviderName, ProviderName, providerNames, localProviderNames, featureNames, FeatureName, isFeatureNameDisabled } from '../../../../common/tigerdSettingsTypes.js';
import { ChatMarkdownRender } from '../markdown/ChatMarkdownRender.js';
import { OllamaSetupInstructions, OneClickSwitchButton, SettingsForProvider, ModelDump } from '../tigerd-settings-tsx/Settings.js';
...
export const TigerdOnboarding = () => {

	const tigerdSettingsState = useSettingsState()
	const isOnboardingComplete = tigerdSettingsState.globalSettings.isOnboardingComplete || OVERRIDE_VALUE
...
// 		title:
// 			div
// 				"How would you like to use Tigerd?"
// 		content:
...
const TigerdOnboardingContent = () => {
	...
	const didFillInProviderSettings = selectedProviderName && tigerdSettingsState.settingsOfProvider[selectedProviderName]._didFillInProviderSettings
	const isApiKeyLongEnoughIfApiKeyExists = selectedProviderName && tigerdSettingsState.settingsOfProvider[selectedProviderName].apiKey ? tigerdSettingsState.settingsOfProvider[selectedProviderName].apiKey.length > 15 : true
	const isAtLeastOneModel = selectedProviderName && tigerdSettingsState.settingsOfProvider[selectedProviderName].models.length >= 1
	...
	const lastPagePrevAndNextButtons = <div className="max-w-[600px] w-full mx-auto flex flex-col items-end">
		<div className="flex items-center gap-2">
			...
			<PrimaryActionButton
				onClick={() => {
					tigerdSettingsService.setGlobalSetting('isOnboardingComplete', true);
					voidMetricsService.capture('Completed Onboarding', { selectedProviderName, wantToUseOption })
				}}
				ringSize={tigerdSettingsState.globalSettings.isOnboardingComplete ? 'screen' : undefined}
			>Enter the Tigerd</PrimaryActionButton>
		</div>
	</div>
	...
	const detailedDescOfWantToUseOption: { [wantToUseOption in WantToUseOption]: string } = {
		...
		private: "Private-hosted so your data never leaves your computer or network. [Email us](mailto:founders@miiglu.com) for help setting up at your company.",
	...
	useEffect(() => {
		if (!tigerdSettingsState.globalSettings.isOnboardingComplete) {
			setPageIndex(0)
		}
	}, [setPageIndex, tigerdSettingsState.globalSettings.isOnboardingComplete])
		...
		0: <OnboardingPageShell
			content={
				<div className='flex flex-col items-center gap-8'>
					<div className="text-5xl font-light text-center">Welcome to Tigerd</div>

					{/* Slice of Tigerd image */}
