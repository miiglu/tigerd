...
import { isFeatureNameDisabled } from '../../../../common/tigerdSettingsTypes.js'
...
export const useApplyStreamState = ({ applyBoxId }: { applyBoxId: string }) => {
	const accessor = useAccessor()
	const tigerdCommandBarService = accessor.get('ITigerdCommandBarService')

	const getStreamState = useCallback(() => {
		const uri = getUriBeingApplied(applyBoxId)
		if (!uri) return 'idle-no-changes'
		return tigerdCommandBarService.getStreamState(uri)
	}, [tigerdCommandBarService, applyBoxId])
...
export const useEditToolStreamState = ({ applyBoxId, uri }: { applyBoxId: string, uri: URI }) => {
	const accessor = useAccessor()
	const tigerdCommandBarService = accessor.get('ITigerdCommandBarService')
	const [streamState, setStreamState] = useState(tigerdCommandBarService.getStreamState(uri))
	// listen for stream updates on this box
	useCommandBarURIListener(useCallback((uri_) => {
		const shouldUpdate = uri.fsPath === uri_.fsPath
		if (shouldUpdate) { setStreamState(tigerdCommandBarService.getStreamState(uri)) }
	}, [tigerdCommandBarService, applyBoxId, uri]))
...
const ApplyButtonsForEdit = ({
	...
	const onClickSubmit = useCallback(async () => {
		...
		if (!applyDonePromise) {
			notificationService.info(`Tigerd Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`)
		}
		...
		applyDonePromise?.catch(e => {
			...
			notificationService.info(`Tigerd Error: There was a problem running Apply: ${e}.`)
