...
import { mountTigerdSettings } from './react/out/tigerd-settings-tsx/index.js'
...
class TigerdSettingsInput extends EditorInput {
	...
	readonly resource = TigerdSettingsInput.RESOURCE;
	...
	override get typeId(): string {
		return TigerdSettingsInput.ID;
	}

	override getName(): string {
		return nls.localize('tigerdSettingsInputsName', 'Tigerd\'s Settings');
	}
...
class TigerdSettingsPane extends EditorPane {
	...
	constructor(
		...
		super(TigerdSettingsPane.ID, group, telemetryService, themeService, storageService);
	}
	...
	protected createEditor(parent: HTMLElement): void {
		...
		this.instantiationService.invokeFunction(accessor => {
			const disposeFn = mountTigerdSettings(settingsElt, accessor)?.dispose;
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(TigerdSettingsPane, TigerdSettingsPane.ID, nls.localize('TigerdSettingsPane', "Tigerd\'s Settings Pane")),
	[new SyncDescriptor(TigerdSettingsInput)]
);


// register the gear on the top right
export const VOID_TOGGLE_SETTINGS_ACTION_ID = 'workbench.action.toggleTigerdSettings'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: VOID_TOGGLE_SETTINGS_ACTION_ID,
			title: nls.localize2('tigerdSettings', "Tigerd: Toggle Settings"),
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// if is open, close it
		const openEditors = editorService.findEditors(TigerdSettingsInput.RESOURCE); // should only have 0 or 1 elements...
		...
		// else open it
		const input = instantiationService.createInstance(TigerdSettingsInput);
...
export const VOID_OPEN_SETTINGS_ACTION_ID = 'workbench.action.openTigerdSettings'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: VOID_OPEN_SETTINGS_ACTION_ID,
			title: nls.localize2('tigerdSettingsAction2', "Tigerd: Open Settings"),
			f1: true,
			icon: Codicon.settingsGear,
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const openEditors = editorService.findEditors(TigerdSettingsInput.RESOURCE);
		if (openEditors.length > 0) {
			await editorService.closeEditors(openEditors);
		}
...
MenuRegistry.appendMenuItem(MenuId.GlobalActivity, {
	...
	command: {
		id: VOID_TOGGLE_SETTINGS_ACTION_ID,
		title: nls.localize('tigerdSettingsActionGear', "Tigerd\'s Settings")
	},
	order: 1
});
