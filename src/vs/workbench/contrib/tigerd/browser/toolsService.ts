...
export class ToolsService implements IToolsService {
	...
	constructor(
		...
		@ITigerdModelService tigerdModelService: ITigerdModelService,
		@IEditCodeService editCodeService: IEditCodeService,
		@ITerminalToolService private readonly terminalToolService: ITerminalToolService,
		@ITigerdCommandBarService private readonly commandBarService: ITigerdCommandBarService,
		...
		this.callTool = {
			read_file: async ({ uri, startLine, endLine, pageNumber }) => {
				await tigerdModelService.initializeModel(uri)
				const { model } = await tigerdModelService.getModelSafe(uri)
		...
		this.stringOfResult = {
			...
			search_in_file: (params, result) => {
				const { model } = tigerdModelService.getModel(params.uri)
			...
			edit_file: (params, result) => {
				const lintErrsString = (
					this.tigerdSettingsService.state.globalSettings.includeToolLintErrors ?
			...
			run_command: (params, result) => {
				...
				if (resolveReason.type === 'timeout') {
					return `${result_}\nTerminal command ran, but was automatically killed by Tigerd after ${MAX_TERMINAL_INACTIVE_TIME}s of inactivity and did not finish successfully. To try with more time, open a persistent terminal and run the command there.`
				}
