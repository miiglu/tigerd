...
import { TigerdFileSnapshot } from '../common/editCodeServiceTypes.js';
...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart Tigerd)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart Tigerd), cancel that last tool
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		...
		const userModifiedTigerdFileSnapshot = fsPath in checkpointMessage.userModifications.voidFileSnapshotOfURI ? checkpointMessage.userModifications.voidFileSnapshotOfURI[fsPath] ?? null : null
		return { voidFileSnapshot: userModifiedTigerdFileSnapshot ?? voidFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const voidFileSnapshotOfURI: { [fsPath: string]: TigerdFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._tigerdModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { voidFileSnapshot: oldTigerdFileSnapshot } = res
			...
			const voidFileSnapshot = this._editCodeService.getTigerdFileSnapshot(URI.file(fsPath))
			if (oldTigerdFileSnapshot === voidFileSnapshot) continue
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getTigerdFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					this._editCodeService.restoreTigerdFileSnapshot(URI.file(fsPath), voidFileSnapshot)
					break
				}
			}
		}
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._tigerdModelService.getModelSafe(uri)
