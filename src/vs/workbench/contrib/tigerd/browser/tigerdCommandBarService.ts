...
import { mountTigerdCommandBar } from './react/out/tigerd-editor-widgets-tsx/index.js'
...
export interface ITigerdCommandBarService {
...
export const ITigerdCommandBarService = createDecorator<ITigerdCommandBarService>('TigerdCommandBarService');
...
export class TigerdCommandBarService extends Disposable implements ITigerdCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'void.TigerdCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._tigerdModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(ITigerdCommandBarService, TigerdCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type TigerdCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountTigerdCommandBar(root, accessor, { uri, editor } satisfies TigerdCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies TigerdCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(ITigerdCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidRejectDiffAction', 'Tigerd: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToNextDiffAction', 'Tigerd: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToPrevDiffAction', 'Tigerd: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToNextUriAction', 'Tigerd: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToPrevUriAction', 'Tigerd: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidAcceptFileAction', 'Tigerd: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidRejectFileAction', 'Tigerd: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidAcceptAllDiffsAction', 'Tigerd: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidRejectAllDiffsAction', 'Tigerd: Reject All Diffs in All Files'),
			keybinding: {
