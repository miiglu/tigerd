 ...
 *--------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------
 *  Tigerd — based on Void Editor by Glass Devtools, Inc.
...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/Tigerd
	...
	constructor(
		...
		super()
		this.client = new PostHog('disabled', {
	...
	async initialize() {
		...
		const { commit, version, tigerdVersion, release, quality } = this._productService
		...
		console.log('User is opted out of basic Tigerd metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('Tigerd posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
