...
export class TigerdMainUpdateService extends Disposable implements ITigerdUpdateService {
	...
	async check(explicit: boolean): Promise<TigerdCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Tigerd to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<TigerdCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/miiglu/binaries/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Tigerd is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Tigerd is up-to-date!'
					}
				}
				else {
