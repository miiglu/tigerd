...
import { FeatureName, ModelSelectionOptions, OverridesOfModel, ProviderName } from './tigerdSettingsTypes.js';
...
export type TigerdStaticModelInfo = { // not stateful
	// Tigerd uses the information below to know how to handle each model.
...
type TigerdStaticProviderInfo = { // doesn't change (not stateful)
	...
	modelOptions: { [key: string]: TigerdStaticModelInfo };
	modelOptionsFallback: (modelName: string, fallbackKnownValues?: Partial<TigerdStaticModelInfo>) => (TigerdStaticModelInfo & { modelName: string, recognizedModelName: string }) | null;
}
...
const defaultModelOptions = {
	...
	supportsFIM: false,
	reasoningCapabilities: false,
} as const satisfies TigerdStaticModelInfo
...
} as const satisfies { [s: string]: Partial<TigerdStaticModelInfo> }




// keep modelName, but use the fallback's defaults
const extensiveModelOptionsFallback: TigerdStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {

	const lower = modelName.toLowerCase()

	const toFallback = <T extends { [s: string]: Omit<TigerdStaticModelInfo, 'cost' | 'downloadable'> },>(obj: T, recognizedModelName: string & keyof T)
		: TigerdStaticModelInfo & { modelName: string, recognizedModelName: string } => {
...
} as const satisfies { [s: string]: TigerdStaticModelInfo }

const anthropicSettings: TigerdStaticProviderInfo = {
...
const openAISettings: TigerdStaticProviderInfo = {
...
const xAISettings: TigerdStaticProviderInfo = {
...
const geminiSettings: TigerdStaticProviderInfo = {
...
const deepseekSettings: TigerdStaticProviderInfo = {
...
const mistralSettings: TigerdStaticProviderInfo = {
...
const groqSettings: TigerdStaticProviderInfo = {
...
} as const satisfies Record<string, TigerdStaticModelInfo>
const googleVertexSettings: TigerdStaticProviderInfo = {
...
const microsoftAzureSettings: TigerdStaticProviderInfo = {
...
const awsBedrockSettings: TigerdStaticProviderInfo = {
...
const vLLMSettings: TigerdStaticProviderInfo = {
...
const lmStudioSettings: TigerdStaticProviderInfo = {
...
const ollamaSettings: TigerdStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
...
const openaiCompatible: TigerdStaticProviderInfo = {
...
const liteLLMSettings: TigerdStaticProviderInfo = { // https://docs.litellm.ai/docs/reasoning_content
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
	modelOptions: {},
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const openRouterSettings: TigerdStaticProviderInfo = {
...
const modelSettingsOfProvider: { [providerName in ProviderName]: TigerdStaticProviderInfo } = {
