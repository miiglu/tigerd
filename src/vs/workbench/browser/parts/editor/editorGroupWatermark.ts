...
/* eslint-disable */ // Void
import { TIGERD_CTRL_K_ACTION_ID, TIGERD_CTRL_L_ACTION_ID } from '../../../contrib/tigerd/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		this.clear();
		const tigerdIconBox = append(this.shortcuts, $('.watermark-box'));
		...
		const update = async () => {
			...
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				tigerdIconBox.appendChild(buttonContainer);

				// Open a folder
				const openFolderButton = h('button')
				openFolderButton.root.classList.add('tigerd-openfolder-button')
				...
				openSSHButton.root.classList.add('tigerd-openssh-button')
				...
				if (recentlyOpened.length !== 0) {
					...
					tigerdIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('tigerd-link')
				...
				const keys = this.keybindingService.lookupKeybinding(TIGERD_CTRL_L_ACTION_ID);
				const dl = append(tigerdIconBox, $('dl'));
				...
				const keys2 = this.keybindingService.lookupKeybinding(TIGERD_CTRL_K_ACTION_ID);
				const dl2 = append(tigerdIconBox, $('dl'));
				...
				// button3.classList.add('tigerd-settings-watermark-button')
